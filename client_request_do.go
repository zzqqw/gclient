...
func (c *Client) DoRequestUnmarshal(ctx context.Context, method string, uri string, data, d any) error {
	...
	if response.IsError() {
		return &RequestError{URI: uri, Method: method, Response: response, StatusCode: response.StatusCode}
	}
...
func (c *Client) DoRequestBytes(ctx context.Context, method string, uri string, data any) ([]byte, error) {
	...
	if response.IsError() {
		return nil, &RequestError{URI: uri, Method: method, Response: response, StatusCode: response.StatusCode}
	}
...
func (c *Client) callRequest(request *http.Request) (response *Response, err error) {
	response = &Response{Request: request, GClient: c}
	for {
